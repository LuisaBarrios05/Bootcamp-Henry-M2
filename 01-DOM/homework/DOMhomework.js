...
function buildToDos(toDos) {
  // Tu código acá:
  //opción larga
/*let arrayTodo = toDos.map(function(todo,index){
...
function displayToDos() {
  ...
  let llamarBuil = buildToDos(toDoItems)//3

  for(let i = 0; i < llamarBuil.length; i++){
    toDoContainer.appendChild(llamarBuil[i])
  }
}
...
if(input.value !== ''){//si el valor de input no es un string vacío hacemos lo siguiente
  let todo = new ToDo(input.value)//1

toDoItems.push(todo)//2

input.value = " "//para que se borre el texto despues de agregar la tarea y quede en balnco para agregar otra.
displayToDos()
  }
}
...
button.addEventListener('click', addToDo)//le paso la función y no la invoco porque no quiero que se ejecute ahora. Se tiene que ejecutar cuando sucede el evento.
